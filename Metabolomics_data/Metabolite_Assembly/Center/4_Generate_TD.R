...
# Set sample name
Sample_Name = "LEO_Center"

# Load in data

peak.2.peak = read.csv("output/LEO_Center_All-Trans_peak.2.peak.csv") # Load in the peak.2.peak file for all/bulk transformations
num.trans = read.csv("output/LEO_Center_All-Trans_num.peak.trans.csv") # Load in the num.peak.trans file for all/bulk transformations

# Create a summary table of transformations
transformation_counts <- as.data.frame(table(num.trans$num.trans.involved.in))
...
net.dist = distances(net) # time consuming 20 min
...
net.dist = net.dist[max.clus, max.clus] # Setting the net dist to that size only; only lost ~409 peaks by doing this with the dereplicated HJ-Andrews set
...
# Store tree
write.tree(tree, paste("output/", Sample_Name, "_TD_UPGMA.tre", sep = ""))
