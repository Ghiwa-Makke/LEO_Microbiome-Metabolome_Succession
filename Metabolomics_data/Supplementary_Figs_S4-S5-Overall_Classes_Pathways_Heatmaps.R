## LEO Surface samples - Metabolomics Data Analyis
## Compound Classes Heatmap (Supplementary figure S4)
## Kegg Pathway Heatmap (Supplementary Figure S5)
...
col_annot <- metadata %>% 
  select(SampleName, Type) %>%
  column_to_rownames(var = "SampleName")
figure_file <- file.path('Output_figures', 'FigureS4.png')
...
col_annot <- metadata %>% 
  select(SampleName, Type) %>%
  column_to_rownames(var = "SampleName")
figure_file <- file.path('Output_figures', 'FigS5.png')
png(figure_file, width = 2200, height = 1000, res = 300)
p <- pheatmap(new_matrix,
         clustering_distance_rows = 'correlation',
         cluster_cols = F,
