...
# Numerical Variables: ----------------------------------------------------
metadata <- read.csv("Input_files/env_metadata.csv")


## All together: -----------------------------------------------------------
explan <- metadata %>%
  column_to_rownames(var = "SampleID") %>%
  select(MossCoverage, Biocrust, MAG_Shannon, WaterContent, pH, C_N)%>%
  rename(Moss_Coverage = MossCoverage) %>%
  rename(Biocrust_Coverage = Biocrust) %>%
  rename(Microbial_Diversity = MAG_Shannon)%>%
...
explan_biotic <- metadata %>%
  column_to_rownames(var = "SampleID") %>%
  select(MossCoverage, Biocrust, MAG_Shannon)%>%
  rename(Moss_Coverage = MossCoverage) %>%
  rename(Biocrust_Coverage = Biocrust) %>%
  rename(Microbial_Diversity = MAG_Shannon)
