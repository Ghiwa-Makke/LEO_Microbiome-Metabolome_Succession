...
data = read.csv("output/Processed_LEO_Center_Data.csv", row.names = 1) # Importing the organismal data  
tree = read.tree("output/LEO_Center_Weighted_All-Trans_UPGMA.tre") # Importing the tree
...
directory_path <- "TWCD_Null_Results/"

# List all files in the specified directory
files <- list.files(path = directory_path, full.names = TRUE)
# 
# 
files = list.files(path = paste("TWCD_Null_Results/", tree.type, "_Null_Results_", sep = ""), pattern = "bMNTD_rep", full.names = T) # Listing files

# files <- list.files(path = paste("TWCD_Null_Results/", tree.type, "_Null_Results_", Sample_Name, "_", tree.type, "_bMNTD_rep", sep = ""),
#                     pattern = paste0("^", "[0-9]+\\.csv$"),
#                     full.names = TRUE)
...
write.csv(bNTI, paste("output/",Sample_Name, "_", tree.type, "_bNTI_", length(files), ".csv", sep = ""), quote = F)
