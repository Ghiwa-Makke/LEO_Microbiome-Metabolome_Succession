...
data = read.csv("output/Processed_LEO_East_Data.csv", row.names = 1) # Importing the organismal data  
tree = read.tree("output/LEO_East_Weighted_All-Trans_UPGMA.tre") # Importing the tree
...
#files = list.files(path = paste("TWCD_Null_Results/", tree.type, "_Null_Results/",sep = ""), pattern = "bMNTD_rep", full.names = #T) # Listing files
